...
function wordsList(string, subStr){
  let redoneStr = string.replace(/[!?.,;:"]/g, '')
    .toLowerCase()
    .split(' ')
    .filter(item => item.indexOf(subStr) !== -1);

  return new Set(redoneStr);
...
function getWeekDay(date) {
  ...
  return days[calendarDate.getDay()];
}
...
function getLocalDay(date) {
  ...
  if (calendarDate.getDay() === 0) return 7;
...
function getDateAgo(date, days) {
  ...
  let MM = calendarDate.getMonth() + 1;
  if (MM < 10) MM = '0' + MM;

  let yyyy = calendarDate.getFullYear();
